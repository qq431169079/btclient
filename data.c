...
// btcache_head 是16M的头，last_piece是最后一个piece
int create_btcache()
{
	int     i;
	Btcache *node, *last;
...
///////////////////////////////////////////////////////////////////////////////////////
	...
	//count即是最后一个piece的slice个数
	int count = file_length % piece_length / (16*1024); //最后的slice不足一个piece
	if(file_length % piece_length % (16*1024) != 0)  count++; //最后不足一个slice
...
int create_files()
{
	...
	if( is_multi_files() == 0 ) {  // 待下载的为单文件
		...
		*fds = open(file_name,O_RDWR|O_CREAT,0777);
		...
		ret = write(*fds,buff,1); //在文件最后写入一个字节,为了在磁盘上预留出空间
		if(ret != 1)  { printf("%s:%d error",__FILE__,__LINE__); return -1; }
				
	} else {  // 待下载的是多个文件				
		// 查看目录是否已创建,若没有则创建
		ret = chdir(file_name); //改变文件目录
		if(ret < 0) { //文件目录不存在，创建
			ret = mkdir(file_name,0777); //创建文件目录
			if(ret < 0)  { printf("%s:%d error",__FILE__,__LINE__); return -1; }
			ret = chdir(file_name); //改变文件目录
			if(ret < 0)  { printf("%s:%d error",__FILE__,__LINE__); return -1; }
		}
		...
		while(p != NULL) {
			...
			ret = write(fds[i],buff,1);       //在最后写入一个字节，预留空间
			if(ret != 1)   {printf("%s:%d error",__FILE__,__LINE__); return -1;}
...
// sequence是slice在btcache缓冲区的位置
// 并且删除peer的请求队列对应的piece
int write_piece_to_harddisk(int sequnce,Peer *peer)
{
	...
	for(i = 0; i < 20; i++)  piece_hash2[i] = pieces[index+i]; //pieces数组由BT种子解释得到
...
int write_btcache_to_harddisk(Peer *peer)
{
	...
	while(p != NULL) {
		...
		if(full_count == slice_count) {
			write_piece_to_harddisk(first_index,peer); //写入piece，并删除peer的请求队列,此时并不break，因为16MB有很多piece
		}
...
int release_read_btcache_node(int base_count)
{
	...
	while(p != NULL) {
		if(count % slice_count == 0)  { used_count = 0; q = p; } //达到了一个piece
...
void clear_btcache_before_peer_close(Peer *peer)
{
	...
	/*把缓冲区btcache_head的所有与index[]相同的piece清除
	 */
	Btcache *p = btcache_head;
	while( p != NULL ) {
