...
int select_unchoke_peer()
{
	...
	for(i = 0, j = 0; i < unchoke_peers.count; i++) {
		p = peer_head;
		while(p != NULL) {
			if(p == unchoke_peers.unchkpeer[i])  break; //没有断开连接（处于peer队列中），而且是unchoke
			p = p->next;
		}
		if(p == NULL)  { unchoke_peers.unchkpeer[i] = NULL; j++; } //在peer队列中找不到，说明已经断开了
	}
	if(j != 0) {  //////////////现在unchkpeer有的时NULL，有的不是NULL，把他们整理一下
		...
		if(p->state==DATA && is_interested(bitmap,&(p->bitmap)) && is_seed(p)!=1) {
			...
			if(i == len) {
				if( index < UNCHOKE_COUNT ) {
					...
					j = get_last_index(now_fast,UNCHOKE_COUNT);  //获取在now_fast中下载速度最慢的，替换掉
