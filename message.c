...
int create_keep_alive_msg(Peer *peer)
{
	...
	memset(buffer,0,4);  //没有编号和负载,为空，只是占用4个字节而已
	peer->msg_len += 4;
	return 0;
}
...
int create_chock_interested_msg(int type,Peer *peer)
{
	...
	memset(buffer,0,5);//与keep_alive 一样，占5个字节而已
	buffer[3] = 1;
	buffer[4] = type;
...
int create_have_msg(int index,Peer *peer)
{
	...
	memset(buffer,0,9);
	//此处不知为何？消息编号？消息内容
	buffer[3] = 5;  //消息，此处可以是0？
	buffer[4] = 4; //消息编号？
	
	//消息负载
	int_to_char(index,c); //负载是buffer[5]->buffer[8],指明下标是index的piece
	buffer[5] = c[0];
	buffer[6] = c[1];
	buffer[7] = c[2];
	buffer[8] = c[3];
...
int create_request_msg(int index,int begin,int length,Peer *peer)
{
	...
	memset(buffer,0,17); //占17个字节
	buffer[3] = 13; //消息长度占4个字节
	buffer[4] = 6;  //消息编号6
	int_to_char(index,c);
...
int create_piece_msg(int index,int begin,char *block,int b_len,Peer *peer)
{
	...
	int_to_char(b_len+9,c); //这里可以memset
	for(i = 0; i < 4; i++)      buffer[i]    = c[i];
	buffer[4] = 7; //消息编号7
	int_to_char(index,c);
	for(i = 0; i < 4; i++)      buffer[i+5]  = c[i];
...
void discard_send_buffer(Peer *peer)
{
	...
	lin.l_onoff  = 1;
	lin.l_linger = 0;  //等待时间是0秒，即立即关闭socket
	lin_len      = sizeof(lin);
