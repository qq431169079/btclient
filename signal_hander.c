...
{
	if(signal(SIGPIPE,SIG_IGN) == SIG_ERR) {      //向一个没有读端的管道写数据所发出的信号，把它忽略掉
		perror("can not catch signal:sigpipe\n");
		return -1;
	}
	
	if(signal(SIGINT,process_signal)  == SIG_ERR) { //安装信号 SIGINT 处理函数
		perror("can not catch signal:sigint\n");
		return -1;
	}
	
	if(signal(SIGTERM,process_signal) == SIG_ERR) { //安装信号 SIGTERM 处理函数,SIGTERM 与kill相同，只不过可以被阻塞
