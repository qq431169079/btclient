...
Peer* add_peer_node();                    // 添加一个peer结点,新peer结点是初始化形成的
int   del_peer_node(Peer *peer);          // 删除一个peer结点,从peer_head删除peer结点
void  free_peer_node(Peer *node);         // 释放一个peer的内存

int   cancel_request_list(Peer *node);    // 撤消当前请求队列,即删除结点node的请求队列，从头结点开始释放
