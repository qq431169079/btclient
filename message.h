...
int create_request_msg(int index,int begin,int length,Peer *peer); //向peer发送索引是index，偏移是begin的请求piece消息
int create_piece_msg(int index,int begin,char *block,int b_len,Peer *peer);//向peer发送piece的消息，内容是block里的
int create_cancel_msg(int index,int begin,int length,Peer *peer); //与request_msg 消息相反
...
int parse_response_uncomplete_msg(Peer *p,int ok_len);  //ok_len表示完整的消息的长度
// 创建响应消息
