...
int http_encode(unsigned char *in,int len1,char *out,int len2)
{
	...
	for(i = 0, j = 0; i < 20; i++, j++) {
		if( isalpha(in[i]) || isdigit(in[i]) )
			out[j] = in[i];
		else {           //非数字或者字母，把其转化为16进制，%是标志，两位 
			out[j] = '%';      //转义符号
			j++;
			out[j] = hex_table[in[i] >> 4]; //高4
			j++;
			out[j] = hex_table[in[i] & 0xf]; //低4
		}
	}
	out[j] = '\0';
...
int prepare_connect_tracker(int *max_sockfd)
{
	...
	while(p != NULL)  { count++; p = p->next; } // count是tracker个数
	tracker_count = count;
	...
	for(i = 0; i < count; i++) {
		...
		sock[i] = socket(AF_INET,SOCK_STREAM,0); //建立socket
		if(sock < 0) {
		...
		if(ht == NULL) {
			...
			tracker[i].sin_port = htons(tracker_port); //设置端口号
			tracker[i].sin_family = AF_INET; //设置TCP/IP协议族
			valid[i] = -1;
		}
	...
	for(i = 0; i < tracker_count; i++) {
		if(valid[i] != 0) {
			...
			// 设置套接字为非阻塞
			flags = fcntl(sock[i],F_GETFL,0); //获取套接字属性
			fcntl(sock[i],F_SETFL,flags|O_NONBLOCK); //设置套接子属性
			// 连接tracker
...
int prepare_connect_peer(int *max_sockfd)
{
	...
	for(i = 0; i < count && p != NULL; i++) {
		peer_sock[i] = socket(AF_INET,SOCK_STREAM,0); //建立socket
		if(peer_sock[i] < 0) { 
			printf("%s:%d socket create failed\n",__FILE__,__LINE__);
			valid[i] = 0;
			p = p->next;
