...
int read_announce_list()
{
	...
	if( find_keyword("13:announce-list",&i) == 0 ) {
		if( find_keyword("8:announce",&i) == 1 ) {
			...
			strncpy(node->announce,&metafile_content[i],len);  //保存的是URL
			node->announce[len] = '\0';
			node->next = NULL;
		...
		while(metafile_content[i] != 'e') {
			...
			if( memcmp(&metafile_content[i],"http",4) == 0 ) {
				...
				if(announce_list_head == NULL)
					...
					p->next = node; // node成为tracker列表的最后一个结点，尾插入
				}
			}
...
int get_file_length()
{
	...
	if(is_multi_files() == 1)  {
		...
		Files *p = files_head;
		while(p != NULL) { file_length += p->length; p = p->next; } //所有文件长度之和
	} else {      //单文件模式
	...
	for(i = 0; i < filesize-8; i++) {
		if( memcmp(&metafile_content[i],"6:length",8) == 0 )
		{
			...
			if(files_head == NULL)
				...
				p = files_head;
				while(p->next != NULL) p = p->next; //尾插入文件信息
				p->next = node;
			}
		}
		if( memcmp(&metafile_content[i],"4:path",6) == 0 )
		{
			...
			i++;        // skip ':'
			p = files_head;    //p指针已经指向最后了，还需再定位一次？是不是有点多余？
			while(p->next != NULL) p = p->next;
...
/*
 * 获取info之后的字典，方法类似于括号匹配
 * 找到d对应的e后，通过开源代码获取这段字符串转为hash值
 */
int get_info_hash()
{
	int   push_pop = 0;
