...
int create_bitfield()
{
	...
	if( (pieces_length/20) % 8 != 0 )  bitmap->bitfield_length++; //不够一个字节保存piece，就多加一个字节
	...
	if(fp == NULL) {  // 若打开文件失败,说明开始的是一个全新的下载
		memset(bitmap->bitfield, 0, bitmap->bitfield_length); //表示所有的piece都还没下载
	} else {
		fseek(fp,0,SEEK_SET);
		for(i = 0; i < bitmap->bitfield_length; i++)
			(bitmap->bitfield)[i] = fgetc(fp);   //读位图
		fclose(fp); 
...
int restore_bitmap()
{
	...
	sprintf(bitmapfile,"%dbitmap",pieces_length);
	fd = open(bitmapfile,O_RDWR|O_CREAT|O_TRUNC,0666);  //0代表8进制 ,O_TRUNC 表示把已经存在的文件内容清零
	if(fd < 0)  return -1;
